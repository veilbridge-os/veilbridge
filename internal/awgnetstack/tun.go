...
// Package awgnetstack is a VENDORED copy of amneziawg-go's tun/netstack package
// (v1.0.4), MIT-licensed, kept verbatim except for ONE patch documented below.
//
// Why vendored: amneziawg-go pins gVisor ~2025-06 where PacketBuffer.IsNil()
...
// Re-derive when bumping amneziawg-go: diff this file against the upstream
// tun/netstack/tun.go; only that one line should differ.
package awgnetstack
...
func (tun *netTun) WriteNotify() {
	pkt := tun.ep.Read()
	if pkt == nil { // PATCH: upstream uses pkt.IsNil(); removed in newer gVisor.
		return
	}
