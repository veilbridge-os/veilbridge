...
(zones, forwarded ports, rules), and port forwards and your own rules can be
added, changed, switched off and removed through the same self-reverting
transaction; the firewall screen is next. VeilBridge does **not** yet manage
static routes, Wi-Fi or clients — keep LuCI around for those. See [Install](#install)
...
| `v0.2` | Platform layer (uci/ubus) with safe apply + rollback, capabilities, live updates, rebuilt panel, router network | 🟡 pre-release `v0.2.0-alpha1`: platform layer, panel, uplink and local network/DHCP done; on `main`: the firewall over the API (reading, port forwards, own rules); firewall screen and static routes in progress |
